...
def TransmissionEstimate(im_path, height, width,kernel):
	caffe.set_mode_cpu()
	...
	net_full_conv.params['conv1'][0].data[...]=kernel
...
if __name__ == '__main__':
	...
	kernel=np.ones((20,3,5,5),dtype="float16")
	#read weight
	f=open('int8_weights.txt','r')
	for n in np.arange(0,20):
		for c in np.arange(0,3):
			for h in np.arange(0,5):
				for w in np.arange(0,5):
					kernel[n][c][h][w]=f.readline()
	f.close()
	print(kernel)	


	te = TransmissionEstimate(im_path, height, width,kernel)
	t = TransmissionRefine(src,te)
	J = Recover(I,t,A,0.1)
	#cv2.imshow('TransmissionEstimate',te)
	#cv2.imshow('TransmissionRefine',t)
	cv2.imshow('Origin',src)
	cv2.imshow('16bit_fixed_point_Dehaze',J)
	cv2.waitKey(0)
